...
//				$("#add-question-img-dialog").dialog("open");
		...
		prepareUploadify : function prepareUploadify(){
			$("#uploadify-img").uploadify({
					  'debug': false,
					  'buttonText': '点击上传',
					  'buttonCursor': 'pointer',
					  'uploader': document.getElementsByTagName(
						  'base')[0].href + 'admin/upload-uploadify-img/',
					  'swf': document.getElementsByTagName('base')[0].href
					  ...
					  'cancelImage': document.getElementsByTagName(
						  'base')[0].href
									 + 'resources/js/uploadify/cancel.png',
...
//					$('#loader').fadeOut(100);
						  $('#maincontent')
							  .load(location.href + ' #maincontent > *');
						  $("#question-upload-img").modal('hide');
					  },
					  onUploadSuccess: function (file, data, response) {
//			            alert('The file ' + file.name + ' was successfully uploaded with a response of ' + response + ':' + data);
						  var fileurl = data;
						  var destination = $(".img-destination input")
							  .val();
						  if (destination == -1) {
...
//			        		$(".question-content textarea").val( textareaval + "<img class=\"question-content-img\" src=\"" +  fileurl + "\">");
							  ...
							  var displayImg = $(".question-content")
								  .find(".diaplay-img");
							  ...
							  var thisopt = $(
								  $(".add-opt-item")[destination]);
							  var displayImg = thisopt.find(".diaplay-img");

							  if (displayImg.length == 0) {
								  thisopt.find(
									  "input.form-question-opt-item").after(
									  "<a href=\"..\\" + fileurl
